...
public class JaxbParser extends ModelParser
{

	private final ObjectMapper objectMapper = new ObjectMapper();

	public JaxbParser(Settings settings, TypeProcessor typeProcessor)
	{
	...
	protected BeanModel parseBean(SourceType<Class<?>> sourceClass)
	{
		...
		if (beanHelper != null)
		{
			for (BeanPropertyWriter beanPropertyWriter : beanHelper.getProperties())
			{
				...
				if (!isInAnnotationFilter)
				{
					...
					if (!isInAnnotationFilter)
					{
						System.out.println("Skipping " + sourceClass.type + "." + beanPropertyWriter.getName()
								+ " because it is missing an annotation from includePropertyAnnotations!");
						continue;
					}
				}
		...
		else if (!sourceClass.type.isInterface() && !Modifier.isAbstract(sourceClass.type.getModifiers())
				&& isSupported(parentJsonTypeInfo = getAnnotationRecursive(sourceClass.type, JsonTypeInfo.class)))
		{
			// this is child class
		...
		return new BeanModel(sourceClass.type, superclass, taggedUnionClasses, discriminantProperty, discriminantLiteral, interfaces, properties,
				null);
	}
	...
	{
		return jsonTypeInfo != null && jsonTypeInfo.include() == JsonTypeInfo.As.PROPERTY
	...
	private BeanHelper getBeanHelper(Class<?> beanClass)
	{
		...
		try
		{
			...
			final DefaultSerializerProvider.Impl serializerProvider2 = serializerProvider1.createInstance(objectMapper.getSerializationConfig(),
	...
	protected EnumModel<?> parseEnum(SourceType<Class<?>> sourceClass)
	{
		...
		final boolean isNumberBased = jsonFormat != null && (jsonFormat.shape() == JsonFormat.Shape.NUMBER
				|| jsonFormat.shape() == JsonFormat.Shape.NUMBER_FLOAT || jsonFormat.shape() == JsonFormat.Shape.NUMBER_INT);
